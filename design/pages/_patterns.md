...
| Sheet | Confirm disable / turn off TOTP / hard delete | Overlay `bg-surface-overlay` + `raised` `shadow-overlay` panel. Actions 44pt. **Separate copy and confirm labels per flow** — never one sheet for disable and delete. Hard-delete body stresses irreversible remove; disable stresses profile kept. Detail: [drivers.md](drivers.md). |
