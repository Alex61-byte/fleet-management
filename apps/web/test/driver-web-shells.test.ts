...
describe("driver web shells (US-10 hub + travel/handover/usage)", () => {
  ...
  it("driver home is hub only: travel/handover/usage links + getDriverTravel", () => {
    ...
    assert.match(home, /getDriverActiveHandover/);
    assert.match(home, /listDriverDailyUsage/);
    assert.match(home, /travelLocked/);
    assert.match(home, /disabled=\{travelLocked\}/);
    assert.match(home, /Complete Handover In before choosing another vehicle/);
    ...
    assert.match(home, /Day open/);
    assert.match(home, /emphasized/);
  ...
  it("travel page loads vehicles and travel selection APIs", () => {
    ...
    assert.doesNotMatch(travel, /Odometer \(/);
