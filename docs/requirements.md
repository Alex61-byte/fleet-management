...
- **Vehicles list filter & sort (US-120):** Company OA can filter list by custody **All / Out / In**; Company and Individual OA can sort by **soonest/furthest** expiration (built-in insurance/inspection/road tax + custom expirations; not registration). Client projection over `GET /v1/vehicles` this slice.
