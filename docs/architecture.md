...
- **Fleet:** vehicles + compliance dates + optional **vehicle.mileage** (OA edit; **travel PUT** + **handover** write-through — [ADR-014](adr/ADR-014-vehicle-mileage.md)/[ADR-015](adr/ADR-015-vehicle-handovers.md); daily usage does not — [ADR-016](adr/ADR-016-driver-daily-usage.md)) + **vehicle handovers** (Out/In, damage images — [ADR-015](adr/ADR-015-vehicle-handovers.md)) + **driver Daily usage** logs ([ADR-016](adr/ADR-016-driver-daily-usage.md)) (master odometer reading; unit derived from country — [ADR-014](adr/ADR-014-vehicle-mileage.md)); expiry computed on read; **optional side appearance images** (paths in Postgres, bytes in **Supabase Storage** via API **S3 gateway** / `OBJECT_STORAGE_*` — [ADR-013](adr/ADR-013-vehicle-side-images.md)). OA notification menu is hybrid client projection (compliance warnings + `GET /v1/service-due` + Company `GET /v1/handovers/open`) — [ADR-017](adr/ADR-017-compliance-notification-menu.md).
